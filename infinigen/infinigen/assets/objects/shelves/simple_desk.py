...
class SimpleDeskBaseFactory(AssetFactory):
    ...
    def get_asset_params(self, i=0):
        ...
        if params.get("top_material", None) is None:
            params["top_material"] = np.random.choice(
                ["white", "black_wood", "glass"], p=[0.2, 0.3, 0.5]
            )
        if params.get("leg_material", None) is None:
            params["leg_material"] = np.random.choice(["white", "black"], p=[0.5, 0.5])
    ...
    def get_material_func(self, params, randomness=True):
        ...
        elif params["top_material"] == "glass":
            params["top_material"] = surface.shaderfunc_to_material(shader_glass)
        else:

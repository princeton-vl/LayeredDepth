...
def snap_surface_normal(normal):
    x, y, z = normal

    # Normalize the input vector (if not already normalized)
    norm = math.sqrt(x * x + y * y + z * z)
    if norm == 0:
        raise ValueError("The input normal has zero length.")
    x /= norm
    y /= norm
    z /= norm

    # Convert to spherical coordinates
    # phi: angle from the positive Z-axis (0 = up, pi/2 = horizontal)
    phi = math.acos(z)
    # theta: azimuth angle in the XY-plane
    theta = math.atan2(y, x)

    # Snap angles to the nearest multiple of 45° (pi/4 radians)
    snap_angle = math.pi / 4.0
    snapped_phi = round(phi / snap_angle) * snap_angle
    snapped_theta = round(theta / snap_angle) * snap_angle

    # Convert the snapped spherical coordinates back to Cartesian coordinates
    snapped_x = math.sin(snapped_phi) * math.cos(snapped_theta)
    snapped_y = math.sin(snapped_phi) * math.sin(snapped_theta)
    snapped_z = math.cos(snapped_phi)
...
def snap_against(scene, a, b, a_plane, b_plane, margin=0, rev_normal=False):
    ...
    plane_normal_a = snap_surface_normal(plane_normal_a)
    plane_normal_b = snap_surface_normal(plane_normal_b)

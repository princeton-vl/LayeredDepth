...
def colorize_int_array(data, color_seed=0):
    ...
    # Find if 0 exists in unique values and assign white to it
    for i, unique_val in enumerate(uniq):
        if np.all(unique_val == 0):  # Check if this unique value is 0
            unique_colors[i] = [255, 255, 255]  # White
        else:
            # Generate random color for non-zero values
            random_state = np.random.RandomState(unique_val[:2].astype(np.uint32) + color_seed)
            hsv_color = colorsys.hsv_to_rgb(
                random_state.uniform(0, 1), 
                random_state.uniform(0.1, 1), 
                1
            )
            unique_colors[i] = (np.array(hsv_color) * 255).astype(np.uint8)

...
@gin.configurable
def init_db(args):
    if args.use_existing and os.path.exists(
        os.path.join(args.output_folder, "scenes_db.csv")
    ):
...
def jobs_to_launch_next(
    ...
    total_running_scenes = len([s for s in scenes if s["num_running"] > 0])

    for scene in scenes:
        if scene["num_running"] == 0:
            if (
                max_running_scene is not None
                and total_running_scenes >= max_running_scene
            ):
                logging.info(
                    f"Total running scenes {total_running_scenes} >= {max_running_scene}, not starting new scenes"
                )
                continue
            total_running_scenes += 1

...
def sample_home_constraint_params():
    return dict(
        ...
        furniture_fullness_pct=uniform(0.65, 1),
        ...
        obj_on_storage_pct=uniform(0.8, 1.0),
...
def home_furniture_constraints():
    ...
    constraints["storage"] = rooms.all(
        lambda r: (
            storage.related_to(r).count().in_range(1, 10)
            + storage_freestanding.related_to(r).count().in_range(1, 10)
        )
    )
    ...
    constraints["rugs"] = rooms.all(
        lambda r: (cl.min_distance_internal(rugs.related_to(r)) >= 0)
    )
    ...
    constraints["wall_decorations"] = rooms.all(
        lambda r: (
            wall_art.related_to(r).count().in_range(0, 20)
    ...
    constraints["plants"] = rooms.all(
        lambda r: (
            big_plants.related_to(r).count().in_range(0, 5)
            * small_plants.related_to(storage.related_to(r)).count().in_range(0, 20)
        )
    )
    ...
    walldesks = wallfurn[shelves.SimpleDeskFactory]
    ...
    constraints["desk"] = rooms.all(
        lambda r: (
            desks.related_to(r).all(
                lambda t: (
                    deskchair.related_to(r).related_to(t).count().in_range(0, 4)
                    * monitors.related_to(t, cu.ontop).count().in_range(0, 2)
    ...
    constraints["ceiling_lights"] = rooms.all(
        lambda r: (ceillights.related_to(r, cu.hanging).count().in_range(1, 10))
    )
    ...
    constraints["bedroom"] = bedrooms.all(
        lambda r: (
            ...
            * walldesks.related_to(r).count().in_range(0, 2)
            * storage.related_to(r).count().in_range(3, 7)
            * storage_freestanding.related_to(r).count().in_range(2, 5)
            * floor_lamps.related_to(r).count().in_range(0, 2)
    ...
    constraints["livingroom"] = livingrooms.all(
        lambda r: (
            ...
            * storage.related_to(r).count().in_range(0, 10)
            ...
            * coffeetables.related_to(r).count().in_range(0, 5)
